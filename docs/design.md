...
Claude Code および Codex の双方で動作する `SKILL.md` として提供する。**外部の専用 LLM API は使用せず**、開発メンバーが日常的に利用しているエージェント自身が対話インターフェースとなる。skill の役割は極めて薄く保ち、生成された質問の提示、回答の `note` への記録、判定ルールの追加提案、作業内容文章の下書き作成のみを担当する。エージェント自身が独自に作業時間を推測したり、質問を捏造したり、回答を勝手に補完したりすることはない。判定ルールの追加提案についても、skill が `projects.toml` を書き換えることはなく、提案の採否と編集は常にユーザーが行う。
...
7. 同一のリポジトリ・ホスト・アプリケーションに対して同じ回答が当月内に 2 回以上必要だった場合は、その回答を自動化できる `projects.toml` のルール（`[[project]]` の `git` / `title` / `url`、`[[category]]` の `bundles` / `domains` / `paths`）を提示し、追加の可否をユーザーに確認する。ルールは過去分にも遡及適用される（6.2 参照）ため、この提案は凍結の前に行う。ファイルの編集はユーザー自身が行い、skill は書き換えない。同じ確認が毎月繰り返されることを防ぐのがこの手順の目的である
8. 日ごとの作業内容文章は、`dayglass evidence --day` から抽出した「依頼と結果」（マスキング済み・上限文字数あり）および該当日の `time` 表を参照して下書きを作成する。プロジェクトごとに 1〜3 行程度にまとめ、リポジトリ名や PR 番号などの識別子は翻訳・変更せずそのまま保持する
9. 下書きした文章をユーザー自身がレビュー・修正する。確認・修正後の文章は `dayglass note --day <日> --summary "..."` により保存する
10. 最後に `dayglass report --freeze --format csv` を実行して提出用データを凍結し、生成されたファイルのパスを示して「内容を確認の上、管理部へ提出してください」と案内して完了する。skill 自体が外部へ自動送信することはない
