...
final class SetupCoordinator {
    ...
    private func installSkill() throws {
        let locations = [
            home.appendingPathComponent(".claude/skills/dayglass-report/SKILL.md"),
            home.appendingPathComponent(".codex/skills/dayglass-report/SKILL.md"),
        ]
        for location in locations {
            try fileManager.createDirectory(at: location.deletingLastPathComponent(), withIntermediateDirectories: true)
            if !fileManager.fileExists(atPath: location.path) {
                try dayglassSkill.write(to: location, atomically: true, encoding: .utf8)
            }
        }
    }
